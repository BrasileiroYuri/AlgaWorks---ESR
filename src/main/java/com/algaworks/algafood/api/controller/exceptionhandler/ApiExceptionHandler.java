...
//		BindingResult bindingResult = ex.getBindingResult();
		...
		String campos = problemFields.stream().map(field -> field.getName()).collect(Collectors.joining(", "));
//		String fiel = problemFields.stream().filter(field -> field.getName().length() < 5).map(t -> t.getName())
//				.collect(Collectors.joining());
//		System.out.println(fiel);

		String detail = String.format("O recurso '%s' não podem ter a(s) propriedade(s) '%s' nula(s).",
